...
function loadPlaces(map) {
    var lat = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : 32.749899;
    var lng = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : -96.834547;
    ...
    _axios2.default.get('/api/stores/near?lat=' + lat + '&lng=' + lng).then(function (res) {
        ...
        markers.forEach(function (marker) {
            marker.addListener('click', function () {
                var html = '<div class="popup"> <a href="/store/' + this.place.slug + '"><img src="uploads/' + (this.place.photo || 'store.png') + '" alt=' + this.place.name + ' /> <div class="popup-title">' + this.place.name + '</div> <div class="popup-address">  ' + this.place.location.address + ' </div></a> </div>';
...
function ajaxStar(e) {
    ...
    _axios2.default.post('' + this.action).then(function (res) {
        var isStarred = _this.stars.classList.toggle('star__button--starred');
        (0, _bling.$)('.star-count').textContent = res.data.stars.length;
        if (isStarred) {
            _this.stars.classList.add('star__button--float');
            setTimeout(function () {
                _this.stars.classList.remove('star__button--float');
            }, 2500);
        }
    }).catch(function (err) {
...
var _star = __webpack_require__(12);
